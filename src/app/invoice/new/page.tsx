  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      const invoice = await addInvoice({
        ...
        sender: { name: "", stellarAddress: publicKey },
        recipient: { name: values.recipientName, stellarAddress: values.recipientAddress },
      });
        ...
        <div className="max-w-3xl mx-auto px-6 py-10">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                ...
                {itemFields.map((field, index) => (
                  <div key={field.id} className="grid grid-cols-12 gap-2 items-start">
                    ...
                    <div className="col-span-2">
                      ...
                      <input {...register(`items.${index}.quantity`)} type="number" min="1" step="1" placeholder="1" className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                    </div>
            ...
            <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Break payment into stages released by milestone</p>
                </div>
                ...
                <div className="mt-4 space-y-3">
                  {milestoneFields.map((field, index) => (
                    <div key={field.id} className="p-4 rounded-xl bg-gray-50 border border-gray-100 space-y-2">
                      ...
                      <button type="button" onClick={() => removeMilestone(index)} className="text-xs text-red-400 hover:text-red-600">Remove milestone</button>
                    </div>
                  ))}
            ...
            <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <textarea {...register("notes")} rows={3} placeholder="Payment terms, project scope, or any additional context..." className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none" />
            </section>

            {/* Submit */}
            <div className="flex items-center justify-between">
              <button type="button" onClick={() => router.back()} className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                Cancel
              </button>
